...
public enum HeapTool {
    public static func capture(pid: pid_t) async throws -> [HeapClassEntry] {
        ...
        guard result.exitCode == 0 else {
            throw MemwatchError.heapFailed(result.stderr.trimmingCharacters(in: .whitespacesAndNewlines))
        }
...
public enum LeaksTool {
    public static func capture(pid: pid_t) async throws -> String {
        ...
        if result.exitCode != 0 && result.stdout.isEmpty {
            throw MemwatchError.leaksFailed(result.stderr.trimmingCharacters(in: .whitespacesAndNewlines))
        }
