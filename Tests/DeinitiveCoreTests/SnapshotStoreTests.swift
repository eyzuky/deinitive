...
final class SnapshotStoreTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("deinitive-tests-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    func testLoadMissingTagThrowsSnapshotNotFound() {
        ...
        XCTAssertThrowsError(try store.load(tag: "does-not-exist")) { error in
            guard let mw = error as? DeinitiveError else {
                XCTFail("expected DeinitiveError, got \(error)")
                return
            }
            switch mw {
